...
import os
import torch
from utility import read_file_from_directory, create_dir


def problem_solver_to_file(path : str = os.path.join('.','ALL_tsp','Uncompressed'),  min_n : int = 50, verbose : bool = False):
...
if __name__ == '__main__':
    p = problem_solver_to_file(verbose = False)
    create_dir(os.path.join('.', 'our_graphs'))
    for g in p:
        torch.save(g, os.path.join('our_graphs', g.name.split(os.sep)[-1].split('.')[0] + '.txt'))
    pass