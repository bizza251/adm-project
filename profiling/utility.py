...
def stats_len_to_gt_len_ratio(model_output, batch):
    out = get_tour_len(batch.coords, model_output.tour) / batch.gt_len.to(batch.coords.device)
    ...
    if args.metrics is not None:
        for metric in args.metrics:
            if metric == 'len_to_gt_len_ratio':
                metrics[metric] = stats_len_to_gt_len_ratio
